...
# file_path = f'lr6(5)_ped_zap_v{variant}.txt'
...
def F(x):
    if (variant == 18):
        ...
        return pow(2, x) / log(2) - 1.5 * pow(x, 2) + 2 * x
...
def solveIntegral(a, b, integral_sum_method, approx_order, alpha, epsilon):
    global count_uses
    global count_uses_dx
    n = 1
    h = get_h(a, b, n)
    ...
    I_2before = 0
    l = 1 / log(alpha)
    delta_r_coef = pow(alpha, approx_order) / (1.0 - pow(alpha, approx_order))
    ...
    while (True):
        ...
        #Вычисляем оценку порядка сходимости. Должно приближаться к approx_order - порядку сходимости.
        if (n != 1 and n != 2):
    ...
    print(f"Количество обращений к функции = {count_uses}", end = "")
    if (count_uses_dx != 0):
...
if __name__ == '__main__':
    ...
    print(f"Заданная точность вычислений = {epsilon}")
    print("\n")
